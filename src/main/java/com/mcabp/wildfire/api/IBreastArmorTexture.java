...
package com.mcabp.wildfire.api;
...
public interface IBreastArmorTexture {
    ...
    //TODO - 1.21: Re-evaluate this codec
    Codec<IBreastArmorTexture> CODEC = RecordCodecBuilder.create(instance -> instance.group(
          WildfireAPI.VECTOR_2I_CODEC.optionalFieldOf("texture_size", DEFAULT_TEXTURE_SIZE)
                .forGetter(IBreastArmorTexture::textureSize),
          WildfireAPI.VECTOR_2I_CODEC.optionalFieldOf("left_uv", DEFAULT_LEFT_UV)
                .forGetter(IBreastArmorTexture::leftUv),
          WildfireAPI.VECTOR_2I_CODEC.optionalFieldOf("right_uv", new Vector2i(-1, -1))
                .forGetter(IBreastArmorTexture::rightUv),
          WildfireAPI.VECTOR_2I_CODEC.optionalFieldOf("dimensions", DEFAULT_DIMENSIONS)
     ...
     *
     * @return A read only {@link Vector2ic} indicating how large the texture file is
     *
     ...
     *
     * @return A read only {@link Vector2ic} indicating how large of an area should be grabbed from the texture sprite to display over the wearer's breasts
     *
     * @apiNote The X value of this should be halved from the total chest size to account for each breast side rendering independently of each other.
     ...
     *
     * @return A read only {@link Vector2ic} indicating the UV to use for the left breast
     *
     ...
     *
     * @return A read only {@link Vector2ic} indicating the UV to use for the right breast
     *
